...
describe("Chapter2-2 > 심화과제 > Virtual DOM과 이벤트 관리", () => {
  let container;
  ...
  describe("renderElement > ", () => {
    ...
    describe("특수한 속성 처리 > ", () => {
      it("className이 props에서 제거될 때 class 속성이 올바르게 제거되어야 한다", () => {
        ...
        expect(container.outerHTML).toBe(
          `<div><div id="test-element">Content</div></div>`,
        );
      });
      ...
      it("boolean type props가 property로 직접 업데이트되어야 한다 (checked)", () => {
        ...
        // checked를 true로 업데이트
        const updatedVNode = (
          <input type="checkbox" id="test-checkbox" checked={true} />
        );
        ...
        expect(container.outerHTML).toBe(
          `<div><input type="checkbox" id="test-checkbox"></div>`,
        );

        // 다시 false로 업데이트
        const uncheckedVNode = (
          <input type="checkbox" id="test-checkbox" checked={false} />
        );
      ...
      it("boolean type props가 property로 직접 업데이트되어야 한다 (disabled)", () => {
        ...
        expect(container.outerHTML).toBe(
          `<div><button id="test-button" disabled="">Click me</button></div>`,
        );
        ...
        expect(container.outerHTML).toBe(
          `<div><button id="test-button">Click me</button></div>`,
        );
      });
      ...
      it("여러 boolean props가 동시에 올바르게 처리되어야 한다", () => {
        ...
        expect(container.outerHTML).toBe(
          '<div><input type="checkbox" id="multi-props-input"></div>',
        );
        ...
        expect(input.readOnly).toBe(true);
        expect(container.outerHTML).toBe(
          '<div><input type="checkbox" id="multi-props-input" readonly=""></div>',
        );
      });
    });
    ...
    describe("updateElement 엣지케이스 > ", () => {
      it("oldChildren이 newChildren보다 많을 때 초과하는 자식들이 제거되어야 한다", () => {
        ...
        expect(container.firstChild.children[1].textContent).toBe(
          "Second Updated",
        );
      });
      ...
      it("역순으로 자식 제거 로직이 올바르게 작동하는지 확인", () => {
        ...
        const initialIds = Array.from(container.firstChild.children).map(
          (child) => child.id,
        );
      ...
      it("중첩된 구조에서 자식 제거가 올바르게 동작해야 한다", () => {
        ...
        expect(container.firstChild.children[0].children[0].textContent).toBe(
          "Updated Nested 1",
        );
      });
